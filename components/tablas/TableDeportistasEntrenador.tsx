...
const TableDeportistas: React.FC<TableProps> = ({ rol }) => {
	...
	const renderUsers = () => {
		...
		return usersToRender.slice(start, end).map((item) => (
			<tr key={item._id}>
				<td className="border-[#1e1e1e] border-[3px] p-3 bg-[#dfdfdf] text-black text-center" id='texto-general'>
			...
			{ready() && (
				<div className="w-80% mx-auto">
					...
					<input
						...
						className="p-2 rounded-[18px] w-[30%] border-[3px] text-center border-black focus:outline-none mb-4 text-black"
						id='texto-general'
					/>
					<table className="w-full">
						<thead>
							<tr>
								<th className="border-[#1e1e1e] border-[3px] p-3 bg-[#cd1919] w-1/4 text-white text-center">NOMBRE</th>
								<th className="border-[#1e1e1e] border-[3px] p-3 bg-[#cd1919] w-1/4 text-white text-center">CLUB</th>
								<th className="border-[#1e1e1e] border-[3px] p-3 bg-[#cd1919] w-1/4 text-white text-center">CATEGORÍA DE PESO</th>
								<th className="border-[#1e1e1e] border-[3px] p-3 bg-[#cd1919] w-1/4 text-white text-center">CORREO</th>
							</tr>
						</thead>
